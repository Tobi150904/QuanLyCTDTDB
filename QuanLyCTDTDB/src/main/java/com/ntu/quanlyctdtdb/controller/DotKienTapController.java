...
public class DotKienTapController {
    ...
    public String chiTiet(@PathVariable Integer id, Model model,
        ...
        // BUG-FIX: MaGV != MaNguoiDung. Truoc day so sanh ud.maNguoiDung voi
        // gvPhuTrach.maGV -> luon FALSE -> GV phu trach KHONG BAO GIO thay
        // form "Nhan xet GV". Resolve qua giangVienRepo.
        boolean laGVPhuTrach = false;
        if (dot.getGvPhuTrach() != null && ud != null && ud.getMaNguoiDung() != null) {
            var gvHienTai = giangVienRepo
                    .findByNguoiDung_MaNguoiDung(ud.getMaNguoiDung()).orElse(null);
            laGVPhuTrach = gvHienTai != null
                    && dot.getGvPhuTrach().getMaGV().equals(gvHienTai.getMaGV());
        }
        // BUG-FIX: NV DN gio la NguoiDung loai DoanhNghiep voi FK doanhNghiep
        // (Phase 7 refactor). Lay caller.maDoanhNghiep va so voi dot.maDN.
        boolean laDNTiepDon = false;
        if (dot.getDoanhNghiep() != null && ud != null && ud.getNguoiDung() != null) {
            String maDnCuaCaller = ud.getNguoiDung().getMaDoanhNghiep();
            String maDnCuaDot = dot.getDoanhNghiep().getMaDoanhNghiep();
            laDNTiepDon = maDnCuaCaller != null && maDnCuaCaller.equals(maDnCuaDot);
        }
    ...
    public String nhanXetDN(@PathVariable Integer id,
        ...
        try {
            ...
            if (!isAdmin && ud != null) {
                String maDnCuaCaller = ud.getNguoiDung() != null
                        ? ud.getNguoiDung().getMaDoanhNghiep() : null;
                DotKienTap dot = dotKTService.findById(id);
                ...
                if (maDnCuaCaller == null || maDnCuaDot == null
                    ...
                    throw new org.springframework.security.access.AccessDeniedException(
                            "Ban khong phai la doanh nghiep tiep don cua dot kien tap nay.");
                }
            }
